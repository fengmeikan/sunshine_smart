...
public class FragmentTabAdapter implements RadioGroup.OnCheckedChangeListener{
    private String TAG = FragmentTabAdapter.class.getName();
    ...
    public FragmentTabAdapter(AppCompatActivity fragmentActivity, List<Fragment> fragments, int fragmentContentId, RadioGroup rgs) {
...
//                    fragment.onStart(); // 启动目标tab的onStart()
    ...
    private void showTab(int idx){
        for(int i = 0; i < fragments.size(); i++){
            ...
            if(idx == i){
//                Log.e(TAG,"显示了第"+i+"个碎片");
                ft.show(fragment);
            }else{
//                Log.e(TAG,"隐藏了第"+i+"个碎片");
                ft.hide(fragment);
            }
            ft.commit();
        }
